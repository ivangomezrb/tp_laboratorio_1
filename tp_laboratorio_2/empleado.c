...
int pedirDatosEmpleado(struct sEmployee* pEmpleado)
{
	int retorno = -1;
	struct sEmployee bEmployee;
	getString(bEmployee.nombre,"Ingrese nombre: \n","Error",0,50,2);
	getString(bEmployee.lastName,"Ingrese Apellido: \n","Error",0,50,2);
	getFloat(&bEmployee.salary,"Ingrese salario: \n","Error",0,100000000000,2);
	getInt(&bEmployee.sector,"Ingrese sector :\n","Error",1,1000000000000,2);
	*pEmpleado = bEmployee;
	return retorno;
}
